...
function execute_sql_in_oracle($sql) {
    ...
    $connection = oci_connect ("gq006", "cqajvy", "gqiannew2:1521/pdborcl");
    if($connection == false){
        // failed to connect
        display_oracle_error_message(null);
        die("Failed to connect");
    }
    $arrayLength = sizeof($sql);
...
function verify_session($sessionid) {
    ...
    if(!($values = oci_fetch_array ($cursor))){
        ...
        die("Invalid client line 97!");
    }

}

function current_user($sessionid)
{
    // lookup the sessionid in the session table to ascertain the clientid
    $sql = "select l.userid, fname, student, admin " .
        "from LoginSession l join Users u " .
